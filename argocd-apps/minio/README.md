# MinIO Operator ArgoCD Application (k3s)

Конфигурация для развертывания MinIO Operator и Tenant через ArgoCD в кластере **k3s**: Traefik (встроенный ingress), домены `*.lab.local`, репозиторий k3s-test. MinIO — S3-совместимое объектное хранилище.
...
1. **StorageClass:** в k3s по умолчанию есть `local-path`. Проверка: `kubectl get storageclass`. Если нужен другой provisioner — настройте его отдельно.
...
- Интегрироваться с приложениями в кластере (S3 API)
...
graph TB
    ...
    subgraph Infrastructure["Infrastructure"]
        Ingress_Op["Traefik (k3s)<br/>minio-operator.lab.local"]
        Ingress_Tenant["Traefik (k3s)<br/>minio.lab.local"]
...
- **MinIO Operator Console**: Веб-интерфейс для управления Operator и Tenants
  - Доступ: `https://minio-operator.lab.local`
...
1. **Operator Console** (`console` сервис, порт 9090)
   - URL: `https://minio-operator.lab.local`
...
- **`operator/application.yaml`**: 
  ...
  - Включает MinIO Operator Console с Ingress на `minio-operator.lab.local`
...
6. **DNS настроен** для доменов:
   ...
   - `minio-operator.lab.local` (Operator Console)
...
Для доступа к MinIO из других подов в кластере используйте:
...
# Проверить Traefik (k3s: в kube-system)
kubectl get pods -n kube-system | grep traefik
